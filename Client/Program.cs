        ...
        static void Main(string[] args)
        {
            ...
            using (AuthenticationProxy authenticationProxy = new AuthenticationProxy(binding, authenticationServiceAddress)) 
            {
                using (CredentialsStoreProxy credentialsStoreProxy = new CredentialsStoreProxy(binding, credentialsStoreAddress))
                {
                    ...
                    do
                    {
                        if (credentialsStoreProxy.State != CommunicationState.Closed || credentialsStoreProxy.State != CommunicationState.Closing)
                        {
                                        ...
                                        try
                                        {
                                            ...
                                            switch (ret)
                                            {
                                                ...
                                                case 1:
                                                    ...
                                                    Task t = Task.Factory.StartNew(() => DoCheck(authenticationProxy,username));
                                                    break;
                                            }
                                        }
                        ...
                        {
                            Console.WriteLine("\t\t Please Run Client Again for further Use");
                            break;
                        }
                    } while (option != "exit" || credentialsStoreProxy.State != CommunicationState.Closed || credentialsStoreProxy.State != CommunicationState.Closing);
            ...
            void DoCheck(AuthenticationProxy proxy, string user)
            {
                while (proxy.State == CommunicationState.Opened)
                {
                    ...
                    {
                        case -4:
                            Console.WriteLine("{0}  Your Account has been timed out for inactivity",user);
                            proxy.Close();
                            break;
                        case -3:
                            Console.WriteLine("{0}  Your Account has been timed out for inactivity", user);
                        ...
                        case -1:
                            ...
                            break;
                        default:
                            Console.WriteLine("Check Done! Everything OK!");
                            Thread.Sleep(30000);
                            break;
